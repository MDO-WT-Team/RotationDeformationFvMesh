...
#include "rotationDeformationFvMesh.H"
#include "foamTime.H"
...
defineTypeNameAndDebug(rotationDeformationFvMesh, 0);

addToRunTimeSelectionTable(dynamicFvMesh, rotationDeformationFvMesh, IOobject);

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void rotationDeformationFvMesh::calcMovingPoints() const
{
	if (debug)
    {
        Info<< "void rotationDeformationFvMesh::calcMovingMasks() const : "
    ...
    if (movingPointsPtr_)
    {
        FatalErrorIn("void rotationDeformationFvMesh::calcMovingMasks() const")
            ...
            forAll (cellAddr, cellI)
            {
                ...
                forAll (curCell, faceI)
                {
                    ...
                    forAll (curFace, pointI)
                    {
                        ...
                        // rotationDeformationFvMesh::update().  HJ, 14/Oct/2010
...
rotationDeformationFvMesh::rotationDeformationFvMesh(const IOobject& io)
:
    dynamicFvMesh(io),
    dict_
    (
    ...
    //movingPointsPtr_(NULL) // from turbo
    motionPtr_(motionSolver::New(*this))
{
    ...
    if (!cs_.inDegrees())
    {
        WarningIn("rotationDeformationFvMesh::rotationDeformationFvMesh(const IOobject& io)")
...
rotationDeformationFvMesh::~rotationDeformationFvMesh()
{
...
const vectorField& rotationDeformationFvMesh::movingPoints() const
{
...
bool rotationDeformationFvMesh::update()
{
    fvMesh::movePoints(motionPtr_->newPoints());
	
	if (testVariable_ !=  time().value())
	{	
		movePoints
		(
			cs_.globalPosition
